...
14. Public website design direction is LOCKED to `78-PUBLIC-WEBSITE-DESIGN-SYSTEM.md`: a **premium German B2B roll-label manufacturer** look — warm ivory/paper backgrounds (`#F7F3EA`/`#FFFCF6`), charcoal headings (`#0B1220`), muted industrial trust-blue primary CTA (`#1F4E79`), champagne/sand accent (`#B8945E`), **editorial typography (Fraunces headings + IBM Plex Sans body via next/font)**, asymmetric visual-first layout, borderless soft-shadow cards, and explicit anti-AI design rules. This **SUPERSEDES** the navy/blue "German B2B SaaS" direction in `76-UI-DESIGN-SYSTEM-GERMAN-B2B.md` (its `#2563EB` Trust-Blue + white/soft-grey palette and the `77-CODEX-UI-REDESIGN-PROMPT.md` look) and the earlier warm terracotta/cream palette. It remains UI-only: preserve all routes/SEO/metadata/Stripe/order/upload/quote flows, keep the `Ratgeber` + `Glossar` hubs in nav, and keep the full navy product-image hero (no stock-photo clutter, no fake SaaS dashboards). No competitor names, no English/Turkish public copy, no "Made in Germany" claim.
