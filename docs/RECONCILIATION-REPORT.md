...
| R-022 | `00-SOURCE-OF-TRUTH.md`, `62-PHASE-1-MVP.md`, `14-AUTH-AND-ACCOUNTS.md`, `19-CUSTOMER-PORTAL-v2.md` | Implementation moved ahead of docs: the customer Supabase Auth + account layer (Track S · S3) was **built and shipped** (incl. prod migration), but SoT still marked it "DEFERRED / not started" + "real customer auth not implemented live", and `62 §6` still listed "customer portal" (plus already-built Stripe/admin/proof/upload) as Phase-1 non-scope. | Reconciled (2026-06-05): SoT Auth status + the deferred-account section updated to **RESOLVED — built via Track S (founder-authorized), single-item model kept (no multi-item cart, per #15), P3 pulled forward by founder direction**; `62 §6` marked **SUPERSEDED** (74 is canonical phase authority, R-007). No breaking contradiction (prices/flags/single-item rules respected). **Remaining:** mild portal duplication — `/konto` (login) overlaps `/de/gespeicherte-druckdaten` (token); tracked as Track S **Task S8b** (consolidate: token view → fallback that links into `/konto`). |
